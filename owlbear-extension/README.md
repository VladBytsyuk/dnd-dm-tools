...
4. Откройте действие расширения и введите текущий код сопряжения.
...
После установки или обновления расширения игрокам нужно один раз обновить вкладку комнаты Owlbear. Action расширения загружается Owlbear для каждого клиента комнаты; у игроков он показывает только публичную инициативу, а у GM — также управление синхронизацией.
...
Production-код сопряжения содержит временный адрес Quick Tunnel. После запуска Obsidian или перезапуска туннеля скопируйте новый код. Install Link остаётся прежним. Старые v1-коды с локальным портом поддерживаются только для совместимости и разработки.
