...
import {
	getOmniResultTypeColor,
	OMNI_RESULT_TYPE_COLORS,
} from "src/ui/layout/omni/OmniResultTypeColor";
...
describe("OmniResultTypeColor", () => {
	it("defines a distinct color for every searchable panel type", () => {
		const colors = SEARCHABLE_PANEL_KEYS.map(getOmniResultTypeColor);

		expect(Object.keys(OMNI_RESULT_TYPE_COLORS)).toHaveLength(
	...
	it("uses the neutral reference color for a panel without search results", () => {
		expect(getOmniResultTypeColor("initiative-tracker")).toBe("#64748B");
	});
});
